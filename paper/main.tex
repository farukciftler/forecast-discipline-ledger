...
is heavily right-skewed --- median \LatMedian{} day, mean \LatMean, maximum
\LatMax{} --- and $\RepK/\ErrN = \RepRate$ of records are repeats of an
...
count. With \NumDays{} days, no forecasting conclusion here should be read as more
...
bimodal in character: the median is \LatMedian{} day while the mean is
\LatMean{} and the maximum is \LatMax. Routine cross-checking catches errors of value almost
immediately; a different kind of error survives it entirely. Second, more than
a third of records ($\RepRate$) are repeats of an earlier record --- the agent
...
($n = \AbsN$) have median latency $\AbsMedian$ days and mean $\AbsMean$; the
remainder ($n = \OthN$) have median $\OthMedian$ and mean $\OthMean$. A
Mann--Whitney test gives $z = \MWZ$, two-sided $p = \MWP$:
\textbf{not supported}. The difference lives
