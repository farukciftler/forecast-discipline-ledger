...
\begin{enumerate}
  ...
  \item Pre-registered null results on short-horizon forecasting, including
        ...
        (\S\ref{sec:forecast}).
  \item A named failure mode, \emph{rules written but not mechanised}, with
        four illustrated instances and a measured repeat rate
        (\S\ref{sec:mechanism}).
...
documentation, and then failed to apply it. A document is not an enforcement
point.
...
applies, and that is the moment the agent is busy with whatever caused the rule
...
forecasts with no demonstrated skill. That half of the result is dull, and it
was pre-registered so that it could not later be reframed as something better.
...
slowest took \LatMax{} days. Over a third repeat an earlier record. The
